      ...
      >
        <el-row>
          ...
          >
            ...
            >
              ...
              <el-input
                ...
                "
                v-model.trim="queryParams.contextData[item.field]"
          ...
          <el-col :span="6" style="text-align: center;margin-top:5px">
            <el-button type="primary" icon="el-icon-search" size="mini" @click="handleQueryForm('query')" style="margin-left:12px"
              >搜索</el-button
            >
            <el-button icon="el-icon-refresh" size="mini" @click="handleResetForm()">重置</el-button>
            <a
...
<script>
import AnjiTree from "@/components/report/AnjiPlus/anji-tree.vue";
...
export default {
  ...
  created() {
    ...
    this.option.queryFormFields.forEach(item => {
      // 动态添加属性
      this.$set(this.queryParams.contextData, item.field, item.defaultValue || '');
    });
    // 查询列表
  ...
  methods: {
    ...
    handleQueryForm(from) {
      ...
      if (from == "query") {
        if (this.hasTreeFieldInQueryForm) {
          delete this.queryParams.contextData[this.queryFormTreeField.field];
        }
      }
      // 如果是点树查询，把查询区里的属性去掉
      if (from == "tree") {
        if (this.hasTreeFieldInQueryForm) {
          var treeVal = this.queryParams.contextData[this.queryFormTreeField.field];
          this.queryParams = {
            pageNumber: 1,
            pageSize: 10
          };
          this.queryParams.contextData[this.queryFormTreeField.field] = treeVal;
        }
      }
      // 默认的排序
      if (
    ...
    async handleQueryPageList() {
      ...
      this.option.columns = comdata
      this.resData = data.data;
      this.total = data.data.length;
      this.records = data.data.slice(this.pageNumber-1,this.pageSize) //当页面第一次加载进来，默认第一页数据


    // <!-----------------------自定义数据------------->
    // let str = JSON.parse(JSON.stringify(data2))
    //   let obj = str[0]
    //   for(var i in obj) {
    //     comdata.push({
    //       label: i,
    ...
    //   this.records = data2.slice(this.pageNumber-1,this.pageSize) //当页面第一次加载进来，默认第一页数据
    },
    // 重置
    handleResetForm() {
      this.queryParams = {
    ...
    handleCurrentChange(val) {
      ...
      // this.handleQueryPageList();
      // 最后一页的页数
      let lastPage = parseInt(this.total / this.pageSize + 1)
      // 最后一页的数量
      let lastNum = this.total % this.pageSize
      this.currentPage = val
      // 页数的起始条数
      let mostCount = val * this.pageSize - this.pageSize
      let pages = []
      for (let m = lastPage === val ? lastNum : this.pageSize; m > 0; m--) {
        pages.push(mostCount)
        mostCount = mostCount + 1
      }
      let pageDatas = []
      pages.forEach((element) => {
        pageDatas.push(this.resData[element])
      })
      this.records = pageDatas
    },
    // 每页size改变时
    handleSizeChange(val) {
      this.pageNumber = 1;
      this.pageSize = val;
      this.queryParams.setCode = this.$route.query.setCode,
      this.handleQueryPageList();
    },
    // table列文件缩略图
    ...
    queryFormChange(fileName, fieldVal) {
      if (typeof this.option.queryFormChange == "function") {
        this.option.queryFormChange(this.queryParams.contextData, fileName, fieldVal);
      }
    }
  }
};
